$(document).ready(function() {
  ...
  let phoneInfos = d3.select("#phone").node().getBoundingClientRect();
  let svgLock = d3.select('#borderLock');
  let borderPad=2.5;
  svgLock.append('rect')
  .attr('id', 'border')
  .attr('x', borderPad)
  ...
  .attr('rx', 15)
  .attr('ry', 15)
  .attr('width', phoneInfos.width-2*borderPad)
  .attr('height', phoneInfos.height-2*borderPad)
  .style('stroke', 'black')
  .style('stroke-width', 5)
  .style('fill','none');
  let lockDiv = document.getElementById("contentLock");
  ...
  datum.style.marginTop = 0.12*phoneInfos.height+"px";
    ...
    lock.checkForPattern('12369',function(){
      ...
      setTimeout(() => {
        ...
        $("#phone").addClass("lockFormat");
        $("#phone").addClass("phoneFormat");
        $("#contentLock").addClass("invisible");
        $("#borderLock").addClass("invisible");
        $("#monTel").removeClass("invisible");
